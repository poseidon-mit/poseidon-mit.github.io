...
export default function ExecuteHistoryPage() {
  ...
  const approvalRate =
    totalDecisions > 0 ? Math.round((approvedCount / totalDecisions) * 100) : 0;
      ...
      >
        ...
        <motion.div variants={fadeUp}>
          <ListHeroBanner
            ...
            stats={[
              ...
              {
                label: "Approval",
                value: totalDecisions > 0 ? `${approvalRate}%` : "-",
                color:
                  approvalRate >= 80
                    ? "var(--state-healthy)"
                    : approvalRate >= 50
              ...
              {
                label: "Savings/mo",
                value: formatUsd(savings.currentMonthlySavingsUsd),
              },
            ]}
          />
        </motion.div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-5">
          ...
          >
            ...
            {(
              ["all", "approved", "deferred", "rejected"] as DecisionFilter[]
            ).map((f) => (
              <button
                key={f}
          ...
          {filteredEvents.length === 0 ? (
            <motion.div variants={fadeUp}>
              <div className="glass-list-card rounded-2xl p-12 flex items-center justify-center">
                <EmptyState
                  icon={History}
                  title={
                    totalDecisions === 0
                  ...
                  description={
                    totalDecisions === 0
                      ? "Actions you approve or defer from the Execute queue will appear here with full audit traceability."
...
function HistoryRow({ event }: { event: EnrichedEvent }) {
  ...
  return (
    <div className="glass-list-card group flex items-center gap-4 rounded-2xl p-5 transition-all hover:-translate-y-0.5 hover:bg-white/[0.04] hover:border-white/[0.12] lg:p-6">
      <div
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          ...
          <span
            className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest border ${badgeCls}`}
          >
            {event.decision}
          </span>
        </div>
        <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
          <span className="font-mono tabular-nums">{event.actionId}</span>
          <span>·</span>
