    ...
    <div className="relative min-h-screen w-full">
      ...
      <motion.div id="main-content" className="mx-auto flex flex-col gap-6 md:gap-8 px-4 py-6 md:px-6 md:py-8 lg:px-8" style={{ maxWidth: '1280px' }} variants={stagger} initial="hidden" animate="visible" role="main">
        ...
        <motion.div variants={fadeUp} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {quickLinks.map((ql) => <
          Button key={ql.title} disabled variant="glass" engine="govern" fullWidth className="rounded-2xl p-4 text-left opacity-60 cursor-not-allowed !h-auto !min-h-0 justify-start">
            ...
            {filteredFaqs.map((faq, idx) => <Surface
              ...
              <div className="px-4 pb-4 border-t border-white/[0.06]">
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      ...
                      <Button onClick={() => setHelpful({ ...helpful, [idx]: true })} variant="ghost" engine="protect" size="sm" className={`p-1 rounded !h-6 !min-h-6 !w-6 !px-0 ${helpful[idx] === true ? 'bg-emerald-500/20 text-emerald-400' : 'text-white/30 hover:text-white/50'}`}>
                        <ThumbsUp className="h-3 w-3" />
                      </Button>
                      <Button onClick={() => setHelpful({ ...helpful, [idx]: false })} variant="ghost" size="sm" className={`p-1 rounded !h-6 !min-h-6 !w-6 !px-0 ${helpful[idx] === false ? 'bg-red-500/20 text-red-400' : 'text-white/30 hover:text-white/50'}`}>
        ...
        <motion.div variants={fadeUp}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {docLinks.map((dl) => <
            Button key={dl.title} disabled variant="glass" engine="govern" fullWidth className="rounded-2xl p-4 text-left opacity-60 cursor-not-allowed !h-auto !min-h-0 justify-start">
        ...
        <Surface variants={fadeUp} className="rounded-2xl p-4 md:p-6" variant="glass" padding="none" as={motion.div}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button disabled variant="glass" engine="govern" size="sm" className="rounded-xl text-sm font-semibold opacity-60 cursor-not-allowed" aria-label="Submit ticket preview only">
