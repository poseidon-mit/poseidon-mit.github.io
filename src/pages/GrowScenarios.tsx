...
export default function GrowScenariosPage() {
  ...
  const activeScenario =
    SCENARIOS.find((s) => s.id === selected) ?? SCENARIOS[1];
        ...
        <motion.div variants={fadeUpVariant}>
          <ListHeroBanner
            ...
            stats={[
              ...
              {
                label: "Current",
                value: `$${RESERVE_CURRENT.toLocaleString()}`,
              },
            ]}
          />
        </motion.div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-6">
          ...
          >
            {SCENARIOS.map((s) => (
              <motion.div
                ...
                className={`glass-list-card relative rounded-2xl p-6 lg:p-8 flex flex-col gap-4 text-left transition-all will-change-transform cursor-pointer ${
                  selected === s.id
                    ? "border-2 border-violet-400 shadow-[0_0_20px_rgba(139,92,246,0.15)] -translate-y-0.5"
              ...
              >
                <div
                  className={`absolute left-0 top-0 bottom-0 w-1 transition-opacity ${selected === s.id ? "opacity-100" : "opacity-0"}`}
                  style={{ background: "var(--engine-grow)" }}
                />

                {selected === s.id && (
                  <div className="absolute top-6 right-6 w-6 h-6 rounded-full flex items-center justify-center z-10 bg-violet-600">
                    <Check size={14} className="text-white" />
                  </div>
                )}
                <div className="flex-1 flex flex-col">
                  <p className="text-xl font-light tracking-wide text-foreground mb-2 pr-8">
                    {s.name}
                  </p>
                  <p className="text-sm text-muted-foreground leading-relaxed tracking-wide mb-6 flex-1">
                    {s.desc}
                  </p>
                  <div className="flex flex-col gap-4 border-t border-white/[0.06] pt-5 mt-auto">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-white/40 tracking-widest uppercase text-xs font-semibold">
                        Monthly
                      </span>
          ...
          <motion.section variants={fadeUpVariant}>
            <div className="glass-detail-card rounded-3xl">
              <div className="p-6 lg:p-10 flex flex-col transition-colors">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-white/[0.06] pb-6 mb-8">
                  <h3 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                    {activeScenario.name} forecast
                  </h3>
                  <span className="text-xs font-mono font-medium tabular-nums text-[var(--engine-grow)] mt-2 md:mt-0 bg-[var(--engine-grow)]/10 px-3 py-1.5 rounded-full border border-[var(--engine-grow)]/20">
                ...
                <div className="flex-1 flex flex-col justify-center gap-4">
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/[0.06]">
                    <span className="text-[10px] font-mono tabular-nums text-muted-foreground uppercase tracking-widest px-2">
                      Now (${RESERVE_CURRENT.toLocaleString()})
                    </span>
                    <span className="text-[10px] font-mono tabular-nums text-muted-foreground uppercase tracking-widest px-2">
                      +12 months ($
                      {activeScenario.data[11].median.toLocaleString()}{" "}
                      projected)
                    </span>
                  </div>
                </div>
          ...
          <motion.section variants={fadeUpVariant}>
            ...
            >
              <div className="p-8 lg:p-10 flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="max-w-2xl pl-2">
                  ...
                  <p className="text-base text-muted-foreground leading-relaxed tracking-wide mt-2">
                    This will queue a monthly contribution of{" "}
                    <span className="font-mono tabular-nums text-[var(--engine-grow)] font-bold text-lg px-2 bg-violet-500/10 rounded-md border border-violet-500/20">
                      ${activeScenario.monthlySave.toLocaleString()}
                    </span>{" "}
                    for approval in Execute.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-4 md:ml-auto">
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      "rounded-2xl px-8 transition-all bg-amber-600 text-white hover:bg-amber-700 hover:shadow-[0_0_20px_rgba(245,158,11,0.3)] font-bold tracking-wide border-none flex items-center gap-2",
                    )}
                  >
          ...
          <motion.section variants={fadeUpVariant} className="mb-4">
            <div className="glass-detail-card rounded-3xl p-8">
              ...
              <p className="text-sm leading-relaxed text-muted-foreground tracking-wide font-light max-w-4xl">
                Forecasts incorporate market volatility, expense variation, and
                income stability. Confidence bands widen at longer horizons. All
                projections are re-evaluated weekly. Historical accuracy of this
                model:{" "}
                <strong className="font-medium text-foreground tracking-wide ">
                  89%
                </strong>{" "}
                within 5% margin.
              </p>
